...
export function TechnikPage() {
  const { t } = useLanguage();
  const [activeMainTab, setActiveMainTab] = useState('PLANTS'); // 'PLANTS' | 'ELEMENTUEBERSICHT'
  ...
  return (
    <div className="space-y-8 pb-16 animate-fade-in">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500">
              Produktionsanlagen, Maschinen-Telemetrie, Instandhaltung und CAD Elementübersicht
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center space-x-3 bg-white p-2 rounded-2xl border border-slate-100 shadow-card">
        <button
        ...
        >
          ...
          <span>Produktionsanlagen &amp; Telemetrie</span>
        </button>

        <button
          onClick={() => setActiveMainTab('ELEMENTUEBERSICHT')}
          className={`flex-1 py-3 px-4 rounded-xl text-xs sm:text-sm font-extrabold transition-all flex items-center justify-center space-x-2 ${
            activeMainTab === 'ELEMENTUEBERSICHT'
              ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/30'
              : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
          }`}
        >
          <FileSpreadsheet className="w-4 h-4" />
          <span>Elementübersicht (Allplan CAD Import)</span>
        </button>
      </div>
