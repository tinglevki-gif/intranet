...
def seed_database(db: Session):
    ...
    if db.query(User).first() is not None:
        logger.info("Datenbank enthält bereits Benutzer. Seeding wird übersprungen (Idempotenz gewährleistet).")
        return

    logger.info("Datenbank ist neu/leer. Starte Initialisierung der Standarddaten...")
    ...
    try:
        ...
        if not existing_menu_keys:
            ...
            db.flush()
            logger.info(f"Seeder: {len(menu_entries)} Standard-Menüeinträge initialisiert.")
        else:
            missing_menus = [
                MenuItem(
                    key=item["key"],
            ...
            if missing_menus:
                ...
                db.flush()
                logger.info(f"Seeder: {len(missing_menus)} neue Menüeinträge ergänzt.")

        # 2. Seed Users (NON-DESTRUCTIVE: Never delete or overwrite existing users!)
        admin_email = "h.senf@tinglev.de"
        ...
        if not existing_admin and db.query(User).first() is None:
            admin_data = corporate_users_data[0].copy()
            pwd = admin_data.pop("password")
            superadmin_user = User(**admin_data, hashed_password=get_password_hash(pwd))
            db.add(superadmin_user)
            db.flush()
            logger.info(f"Seeder: SuperAdmin-Konto '{admin_email}' erfolgreich erstellt.")
        ...
        if db.query(Event).first() is None:
            ...
            db.flush()
            logger.info(f"Seeder: {len(demo_events)} Kalenderevents initialisiert.")

        # 4. Seed Documents and AI Search Chunks (only if empty)
        if db.query(Document).first() is None:
            sample_docs_data = [
                {
            ...
            db.flush()
            logger.info(f"Seeder: {len(sample_docs_data)} Dokumente & RAG-Chunks initialisiert.")

        # 5. Seed Announcements / Mitteilungszentrale (only if empty)
        if db.query(Announcement).first() is None:
            demo_announcements = [
                Announcement(
...
Bitte gebt bis zum 10. April eure Rückmeldung im Mitarbeiterportal ab, damit das Catering entsprechend planen kann.""",
            ...
            db.flush()
            logger.info(f"Seeder: {len(demo_announcements)} Mitteilungen initialisiert.")
        ...
        if db.query(Ticket).first() is None:
            demo_tickets = [
                Ticket(
                    ...
                    ersteller_id=admin_id,
                    zugewiesen_an_id=admin_id,
            ...
            demo_messages = [
                ...
                TicketMessage(
                    ticket_id=demo_tickets[0].id,
                    autor_id=admin_id,
                    nachricht="Hallo, ich habe den 2FA-Token im Admin-Portal zurückgesetzt. Bitte melde dich kurz per Teams für den neuen QR-Code.",
                    ist_interne_notiz=False,
            ...
            db.flush()
            logger.info(f"Seeder: {len(demo_tickets)} Tickets & Nachrichten initialisiert.")

        # 7. Seed External Outlook / iCal Calendar Sources (only if empty)
        if db.query(ExternalCalendarSource).first() is None:
...
END:VCALENDAR"""
            ...
            db.flush()
            logger.info(f"Seeder: {len(demo_sources)} Kalenderquellen initialisiert.")

        # 8. Seed Canteen Weekly Menus (KW 35 & KW 36, 2026)
        if db.query(WeeklyMenu).first() is None:
            kw35_days = [
                {
                    "tag": "Montag",
                    "datum": "2026-08-24",
                    "gericht_haupt": {
                        ...
                        "beschreibung": "Knusprig paniertes Hähnchenbrustfilet mit Zitrone, Pommes Frites oder Kartoffelsalat und Preiselbeeren.",
                        "preis": "6,90 €",
                        "kalorien": "740 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        ...
                        "beschreibung": "Mit Zucchini, Auberginen, frischer Tomatensauce und Rucola-Topping.",
                        "preis": "5,80 €",
                        "kalorien": "520 kcal",
                        "is_vegan": False,
                        "is_vegetarian": True
                ...
                },
                {
                    ...
                    "datum": "2026-08-25",
                    "gericht_haupt": {
                        "titel": "Dänischer Rinderbraten an Rosmarin-Jus",
                        "beschreibung": "Zart geschmorter Rinderbraten mit Apfel-Rotkohl und hausgemachtem Kartoffelgratin.",
                        "preis": "7,50 €",
                        "kalorien": "680 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Cremiges Waldpilz-Risotto",
                        "beschreibung": "Frische Pfifferlinge und Kräuterseitlinge mit Grana Padano und frischem Thymian.",
                        "preis": "6,20 €",
                        "kalorien": "490 kcal",
                        "is_vegan": False,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Frischer Obstsalat mit Minze",
                        "preis": "1,80 €"
                    },
                ...
                },
                {
                    ...
                    "datum": "2026-08-26",
                    "gericht_haupt": {
                        "titel": "Hausgemachte Lasagne al Forno",
                        "beschreibung": "Mit 100% Rinderhackfleisch, Béchamelsauce und herzhaftem Gouda überbacken.",
                        "preis": "6,80 €",
                        "kalorien": "690 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Süßkartoffel-Kichererbsen-Curry (Vegan)",
                        "beschreibung": "Mit Kokosmilch, frischem Spinat, Koriander und duftendem Basmatireis.",
                        "preis": "5,90 €",
                        "kalorien": "450 kcal",
                        "is_vegan": True,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Tiramisu im Glas",
                        "preis": "2,00 €"
                    },
                    "allergene_zusatzstoffe": ["A", "F", "G"]
                },
                {
                    "tag": "Donnerstag",
                    "datum": "2026-08-27",
                    "gericht_haupt": {
                        "titel": "Gebratenes Lachsfilet auf Blattspinat",
                        "beschreibung": "Atlantik-Lachsfilet mit Zitronen-Dill-Sauce und Petersilienkartoffeln.",
                        "preis": "8,20 €",
                        "kalorien": "560 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Gefüllte Paprika mit Quinoa & Gemüse (Vegan)",
                        "beschreibung": "Auf fruchtigem Tomatenragout, serviert mit Kräuter-Couscous.",
                        "preis": "5,80 €",
                        "kalorien": "410 kcal",
                        "is_vegan": True,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Schokoladenmousse mit Sahnehäubchen",
                        "preis": "1,80 €"
                    },
                ...
                },
                {
                    ...
                    "datum": "2026-08-28",
                    "gericht_haupt": {
                        "titel": "Tinglev Gourmet Burger & Süßkartoffel-Pommes",
                        "beschreibung": "180g Angus Beef, Bacon, Cheddar, karamellisierte Zwiebeln und BBQ-Dip.",
                        "preis": "7,80 €",
                        "kalorien": "820 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Knuspriger Falafel-Wrap",
                        "beschreibung": "Mit hausgemachtem Kichererbsen-Hummus, Minz-Joghurtsauce und buntem Salat.",
                        "preis": "5,60 €",
                        "kalorien": "480 kcal",
                        "is_vegan": False,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Panna Cotta mit Erdbeerspiegel",
                        "preis": "1,80 €"
                    },
                    "allergene_zusatzstoffe": ["A", "C", "G", "N"]
                }
            ]
            ...
            kw36_days = [
                {
                    ...
                    "datum": "2026-08-31",
                    "gericht_haupt": {
                        "titel": "Zartes Schweinefilet an Champignon-Rahmsauce",
                        "beschreibung": "Mit schwäbischen Eierspätzle und glacierten Fingermöhrchen.",
                        "preis": "7,40 €",
                        "kalorien": "650 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Bio-Dinkel-Pasta mit Pesto Verde (Vegan)",
                        "beschreibung": "Mit gerösteten Pinienkernen, Kirschtomaten und frischem Basilikum.",
                        "preis": "5,80 €",
                        "kalorien": "480 kcal",
                        "is_vegan": True,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Rote Grütze mit Vanillesauce",
                        "preis": "1,80 €"
                    },
                ...
                },
                {
                    ...
                    "datum": "2026-09-01",
                    "gericht_haupt": {
                        "titel": "Herzhaftes Hirschgulasch mit Waldpilzen",
                        "beschreibung": "Mit Preiselbeer-Birne und traditionellen Semmelknödeln.",
                        "preis": "8,50 €",
                        "kalorien": "710 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Kürbis-Gnocchi in Salbeibutter",
                        "beschreibung": "Mit gerösteten Kürbiskernen und feinem Parmesan.",
                        "preis": "6,20 €",
                        "kalorien": "510 kcal",
                        "is_vegan": False,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Zitronen-Sorbet mit Beeren",
                        "preis": "1,80 €"
                    },
                    "allergene_zusatzstoffe": ["A", "C", "G"]
                },
                {
                    ...
                    "datum": "2026-09-02",
                    "gericht_haupt": {
                        "titel": "Hähnchen-Cordon-Bleu mit Käse-Schinken-Füllung",
                        "beschreibung": "Mit feinen Buttererbsen und knusprigen Kroketten.",
                        "preis": "7,20 €",
                        "kalorien": "730 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Asiatische Wok-Pfanne mit Tofu (Vegan)",
                        "beschreibung": "Knackiges Marktgemüse mit Erdnusssauce und Mie-Nudeln.",
                        "preis": "5,90 €",
                        "kalorien": "440 kcal",
                        "is_vegan": True,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Mangomousse mit Passionsfrucht",
                        "preis": "1,80 €"
                    },
                    "allergene_zusatzstoffe": ["A", "E", "F", "G"]
                },
                {
                    "tag": "Donnerstag",
                    "datum": "2026-09-03",
                    "gericht_haupt": {
                        "titel": "Kabeljaufilet in Senf-Dill-Sauce",
                        "beschreibung": "Mit Gurkensalat und gedämpften Petersilienkartoffeln.",
                        "preis": "7,90 €",
                        "kalorien": "520 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Auberginen-Auflauf (Parmigiana)",
                        "beschreibung": "Mit Mozzarella und frischem Oregano überbacken.",
                        "preis": "6,00 €",
                        "kalorien": "470 kcal",
                        "is_vegan": False,
                        "is_vegetarian": True
                    ...
                    "dessert_beilage": {
                        "titel": "Klassische Crème Brûlée",
                        "preis": "2,00 €"
                    },
                    "allergene_zusatzstoffe": ["A", "D", "G", "M"]
                },
                {
                    "tag": "Freitag",
                    "datum": "2026-09-04",
                    "gericht_haupt": {
                        "titel": "Currywurst Spezial mit hausgemachter Currysauce",
                        "beschreibung": "Mit knusprigen Pommes Frites und Röstzwiebeln.",
                        "preis": "6,50 €",
                        "kalorien": "760 kcal",
                        "is_vegan": False,
                        "is_vegetarian": False
                    ...
                    "gericht_vegetarisch_vegan": {
                        "titel": "Veganes Kichererbsen Tikka Masala",
                        "beschreibung": "Mit Cashewkernen, Kokos-Joghurt und warmem Naan-Brot.",
                        "preis": "5,80 €",
                        "kalorien": "460 kcal",
                        "is_vegan": True,
                        "is_vegetarian": True
                    ...
                    "allergene_zusatzstoffe": ["A", "C", "G", "H"]
                }
            ]
            ...
            menus = [
                WeeklyMenu(
                    calendar_week=35,
                    year=2026,
                    valid_from=datetime.strptime("2026-08-24", "%Y-%m-%d").date(),
                    valid_to=datetime.strptime("2026-08-28", "%Y-%m-%d").date(),
                    days_data=kw35_days,
                    pdf_url=None,
                    is_published=True,
                    erstellt_von_id=admin_id,
                    aktualisiert_am=datetime.utcnow()
                ),
                WeeklyMenu(
                    calendar_week=36,
                    year=2026,
                    valid_from=datetime.strptime("2026-08-31", "%Y-%m-%d").date(),
                    valid_to=datetime.strptime("2026-09-04", "%Y-%m-%d").date(),
                    days_data=kw36_days,
                    pdf_url=None,
                    is_published=True,
                    erstellt_von_id=admin_id,
            ...
            db.add_all(menus)
            db.flush()
            logger.info(f"Seeder: {len(menus)} Speisepläne initialisiert.")

        db.commit()
        logger.info("Datenbank-Initialisierung / Seeding erfolgreich abgeschlossen.")
    except Exception as e:
        db.rollback()
        logger.error(f"Fehler bei der Datenbank-Initialisierung (Rollback ausgeführt): {e}", exc_info=True)
        raise
...
def seed_default_user_avatars(db: Session):
    """
    Safely ensures default corporate users have avatars if they do not already have one set.
    """
    updated = False
